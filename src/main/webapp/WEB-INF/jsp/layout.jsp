...
<div class="layui-layout layui-layout-admin">
	<div class="layui-header">
		...
		<ul class="layui-nav layui-layout-left" lay-filter="test">
			<c:forEach items="${listMap}" var="dataMap">
				<li class="layui-nav-item">
					<a href="javascript:;"><i class="layui-icon">
						<c:if test="${dataMap['authId']==1}">&#xe63c;</c:if>
						<c:if test="${dataMap['authId']==2}">&#xe62a;</c:if>
						<c:if test="${dataMap['authId']==3}">&#xe770;</c:if>
						<c:if test="${dataMap['authId']==4}">&#xe68e;</c:if>
						<c:if test="${dataMap['authId']==5}">&#xe716;</c:if>
						<c:if test="${dataMap['authId']==6}">&#xe63a;</c:if>
					</i>${dataMap['authName']}</a>
					<dl class="layui-nav-child" style="padding-left: 15px">
						<c:forEach items="${dataMap['secAuths']}" var="secAuth">
							<dd>
								<a class="hp-tab-add" hp-href="${secAuth.authorityUrl}" href="javascript:;" >${secAuth.authorityName}</a>
							</dd>
						</c:forEach>
					</dl>
				</li>
			...
			<ul class="layui-nav hp-nav-none">
				<li class="layui-nav-item">
					<a href="javascript:;"  class="hp-user-name">
						<img src="http://q1itv10rj.bkt.clouddn.com/touxiang" class="layui-circle-img layui-anim-upbit">
						...
						<div carousel-item style="height: 750px;">
							<div>
								<div class="layui-bg-green demo-carousel">
									<p style="font-size: 30px;">杨氏集团酒店管理系统</p>
